...
export default function IndustrialLandTransferOutsideKCNSection() {
  ...
  // Filter: Chuyển nhượng đất ngoài KCN (có land_price và code bắt đầu bằng INL-CCN)
  const transferOutsideKCN = sampleIndustrialParks
    .filter(park => park.land_price && park.code.startsWith('INL-CCN'))
    .slice(0, 6) // Lấy 6 sản phẩm đầu tiên

  const cards = transferOutsideKCN.map((park, index) => {
          ...
          >
            ...
            >
              ...
              <div className={`relative flex-shrink-0 ${isPortrait ? 'w-full' : ''}`} style={!isPortrait ? { 
                ...
                <img
                  src={card.thumbnail || "https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?q=80&w=800"}
                  alt={card.title}
