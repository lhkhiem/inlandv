...
export default function IndustrialLeaseSection() {
  ...
  // Filter: Cho thuê (có rental_price_min/max)
  const leaseParks = sampleIndustrialParks
    .filter(park => park.rental_price_min && park.rental_price_max)
    .slice(0, 6) // Lấy 6 sản phẩm đầu tiên

  const cards = leaseParks.map((park, index) => {
    // Convert rental_price từ VND/m²/tháng sang USD/m²/tháng (tỷ giá ~24,000 VND/USD)
          ...
          >
            ...
            >
              ...
              <div className={`relative flex-shrink-0 ${isPortrait ? 'w-full' : ''}`} style={!isPortrait ? { 
                ...
                <img
                  src={card.thumbnail || "https://images.unsplash.com/photo-1581090700227-1e37b190418e?q=80&w=800"}
                  alt={card.title}
