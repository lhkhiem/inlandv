...
import { buildApiUrl, getAssetUrl, getThumbnailUrl } from '@/lib/api';
...
export default function MediaLibraryPage() {
  ...
  // Dropzone for file upload - using native drag and drop
  ...
  const fetchFolders = async () => {
    try {
      const response = await fetch(buildApiUrl('/assets/folders'), {
        credentials: 'include',
      });
  ...
  const fetchMedia = async () => {
    try {
      ...
      const response = await fetch(buildApiUrl(`/assets?${params}`), {
        credentials: 'include',
      });
  ...
  const handleFileUpload = async (files: File[] | FileList | null) => {
    ...
    try {
      for (let i = 0; i < fileArray.length; i++) {
        ...
        // Calculate timeout based on file size (2 minutes per 100MB + 1 minute base)
        const fileSizeMB = fileArray[i].size / 1024 / 1024;
        const timeoutMs = Math.max(60000, 120000 * (fileSizeMB / 100)) + 60000; // min 2 minutes, add 1 minute per 100MB
        ...
        const uploadUrl = buildApiUrl('/assets/upload');
        console.log('[upload] Upload URL:', uploadUrl);
        console.log('[upload] File:', fileArray[i].name, `(${(fileArray[i].size / 1024 / 1024).toFixed(2)}MB)`);
        console.log('[upload] Timeout:', timeoutMs, 'ms');
        ...
        try {
          ...
          if (!response.ok) {
            ...
            try {
              ...
              try {
                ...
                console.error('[upload] Server error text:', errorText);
              } catch (e2) {
          ...
          if (fetchError.name === 'AbortError' || abortController.signal.aborted) {
            throw new Error(`Upload timeout cho file "${fileArray[i].name}". File có thể quá lớn hoặc kết nối bị chậm. Vui lòng thử lại với file nhỏ hơn.`);
          }
      ...
      if (error.message) {
        ...
        errorMessage = 'Upload bị timeout. File có thể quá lớn hoặc kết nối chậm. Vui lòng thử lại.';
      }
  ...
  const handleUrlUpload = async () => {
    ...
    setUploading(true);
    try {
      const response = await fetch(buildApiUrl('/assets/upload/by-url'), {
        method: 'POST',
  ...
  const handleRenameFolder = async (folderId: string) => {
    ...
    try {
      const response = await fetch(buildApiUrl(`/assets/folders/${folderId}`), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  const handleRenameFile = async (fileId: string) => {
    ...
    try {
      const response = await fetch(buildApiUrl(`/assets/${fileId}/rename`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
