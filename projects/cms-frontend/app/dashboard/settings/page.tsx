...
export default function SettingsPage() {
  ...
  const [email, setEmail] = useState<any>({ smtpHost: '', smtpPort: 587, encryption: 'tls', fromEmail: '', fromName: 'Inland CMS', username: '', password: '', enabled: false });
  ...
  const fetchNs = async (ns: string, setter: (v: any)=>void) => {
    try {
      const res = await axios.get<any>(buildApiUrl(`/settings/${ns}`), { withCredentials: true });
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (ns === 'appearance') {
        ...
        await axios.put(buildApiUrl('/settings/appearance'), { namespace: 'appearance', value: appearancePayload }, { withCredentials: true });
        await axios.put(buildApiUrl('/settings/homepage_metrics'), { namespace: 'homepage_metrics', value: homepageMetrics }, { withCredentials: true });
        ...
        await axios.put(buildApiUrl(`/settings/${ns}`), { namespace: ns, value: target.state }, { withCredentials: true });
  ...
  const onClearCache = async () => {
    try {
      await axios.post(buildApiUrl('/settings/clear-cache'), {}, { withCredentials: true });
  ...
  const onResetDefaults = async () => {
    try {
      const res = await axios.post<any>(buildApiUrl('/settings/reset-default'), { scope: activeTab }, { withCredentials: true });
  ...
  const handleClientAssetSelect = async (assetId: string | null, type: 'logo' | 'favicon') => {
    try {
      ...
      if (assetId) {
        const res = await axios.get<any>(buildApiUrl(`/assets/${assetId}`), { withCredentials: true });
      ...
      await axios.put(buildApiUrl('/settings/appearance'), { namespace: 'appearance', value: backendData }, { withCredentials: true });
  ...
  const handleAssetSelect = async (assetId: string | null, type: 'logo' | 'favicon') => {
    try {
      ...
      const currentRes = await axios.get<any>(buildApiUrl('/settings/appearance'), { withCredentials: true });
      ...
      const response = await axios.put<any>(buildApiUrl('/settings/appearance'), { namespace: 'appearance', value: backendData }, { withCredentials: true });
      ...
      console.error('Error details:', {
        ...
        url: buildApiUrl('/settings/appearance'),
      });
              ...
              <div className="space-y-4">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input 
                      type="url" 
                      value={general.businessInfo?.googleMapLink || general.businessInfo?.google_maps_link || ''} 
                      onChange={(e)=>setGeneral({ ...general, businessInfo: { ...general.businessInfo, googleMapLink: e.target.value, google_maps_link: e.target.value } })} 
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      <strong>Khuyến nghị:</strong> Sử dụng Embed URL từ Google Maps (Share → Embed a map). 
                      <br />
                      Short link (maps.app.goo.gl) không thể hiển thị trực tiếp trong iframe.
                    </p>
                  </div>
                </div>
