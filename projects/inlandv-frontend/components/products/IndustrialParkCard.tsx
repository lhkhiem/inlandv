          ...
          <div className="p-5 flex-1 flex flex-col">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="flex items-center gap-2 text-sm">
                ...
                <div>
                  <div className="text-[#2E8C4F] text-xs">Tổng diện tích</div>
                  <div className="font-semibold text-[#2E8C4F]">{formatArea(park.total_area)}</div>
                </div>
              </div>
              {park.available_area !== undefined && park.available_area > 0 && (
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <div>
                    <div className="text-[#2E8C4F] text-xs">Còn trống</div>
                    <div className="font-semibold text-[#2E8C4F]">{formatArea(park.available_area)}</div>
                  </div>
                </div>
              )}
            </div>
            ...
            {park.allowed_industries && park.allowed_industries.length > 0 && (
              <div className="mb-4">
                <div className="text-xs text-[#2E8C4F] mb-1">Ngành nghề:</div>
                <div className="flex flex-wrap gap-1">
                  ...
                  {park.allowed_industries.length > 3 && (
                    <span className="text-xs px-2 py-0.5 bg-gray-100 text-[#2E8C4F] rounded-full">
                      +{park.allowed_industries.length - 3}
                    </span>
                  )}
                </div>
            ...
            {/* Description */}
            {park.description && (
              <p className="text-sm text-[#2E8C4F] line-clamp-2 mb-4">{park.description}</p>
            )}
              ...
              <div>
                {park.rental_price_min && park.rental_price_max && (
                  <>
                    ...
                    <div className="text-xs text-[#2E8C4F] mt-0.5">đ/m²/tháng</div>
                  </>
                )}
                {(!park.rental_price_min || !park.rental_price_max) && (
                  <div className="text-sm text-[#2E8C4F]">Liên hệ để biết giá</div>
                )}
              </div>
