  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const serviceLabels: Record<string, string> = {
        'mua-ban': 'Mua/Bán Bất động sản',
        'thue-cho-thue': 'Thuê/Cho thuê Bất động sản',
        'tu-van-khac': 'Tư vấn khác',
      }

      // Build message with service and user message
      const messageParts: string[] = []
      if (formData.service) {
        messageParts.push(`Dịch vụ: ${serviceLabels[formData.service] || formData.service}`)
      }
      if (formData.message) {
        messageParts.push(`Lời nhắn: ${formData.message}`)
      }
      ...
      const leadData: Lead = {
        ...
        message: fullMessage || 'Yêu cầu tư vấn',
        source: 'contact',
      }
