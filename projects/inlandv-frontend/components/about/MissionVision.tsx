...
export default function MissionVision({ section }: MissionVisionProps = {}) {
  ...
  let sectionData: {
    ...
    subtitle: 'Những điểm mạnh tạo nên uy tín và sự tin cậy của INLANDV trong lĩnh vực bất động sản công nghiệp',
    items: [
      {
        icon: 'Award',
    ...
    backgroundImage: '/images/processed-image-12-13.webp',
  };
  ...
  const items = sectionData.items?.map(item => ({
    ...item,
    icon: iconMap[item.icon || 'Award'] || Award,
  })) || []
            ...
            <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-2">
              {sectionData.title.includes('Inlandv') ? (
                <>
                  ...
                  <span className="text-[#2E8C4F]">Inlandv</span>
                  {sectionData.title.split('Inlandv')[1]}
                </>
              ) : (
                sectionData.title
              )}
